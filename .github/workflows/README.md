...
|----------|---------|---------|
| `ci.yml` | Pushes and pull requests targeting `master`; manual dispatch | Checks that the stale `deploy/server` tree is absent, runs the server-render sync check, the DoD gate (`npm run dod:check`), TypeScript lint/typecheck, and unit tests. Manual runs also execute the Playwright smoke suite and short render fixture gate. |
