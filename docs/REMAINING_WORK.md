...
Last updated: **2026-08-05T12:02Z** — housing max-raw **7.0** upload **NO**; healthcare **6.6**. Dailymotion junk-match fixed (`d15a0ad`, ddg=82 proven on housing-web152) but assemble still collapsed: yt-dlp lacked `curl_cffi==0.13.0` impersonation → every DM download failed → two-still slideshow (raw 5.2). Fixed: pin curl_cffi + DM soft-probe circuit (mirror Vimeo) + DM video-id dedupe + Gwyneth ski-crash reject. Durable requires upload YES. Loop continuing (wave22).
...
**housing-web85 root cause found + fixed (code fix, no fresh watch yet):** raw tip-best 7.0 (LLM variance 7.0/5.6/3.8) was earned by a video that is **not upload-ready** — the LLM watcher correctly flagged it as "a slideshow of 2 static images" (a real U-Haul moving-day photo + a real news photo of a family on a couch, plus a shortform.com AI-illustration "Evicted" door pad). `harvest-quality.json` reported `videoCount` 5–7/segment and `pass: true`, but every one of the 18 injected Vimeo clips was **never probed** before being trusted (`resolveInjectClipProbe` treats all `/api/download-clip` proxy clips as trusted, matching the harvest keep-path). At render time, `yt-dlp` failed all 18 Vimeo fetches with `"blocked due to its TLS fingerprint … your IP may be blocked"`, and `ffmpegAssembly.mjs`'s `pushClip` fallback chain silently reused a handful of already-encoded still images (Ken-Burns zoom) for every failed slot — with zero signal in `render-manifest.json` (`placeholderClipCount: 0`; only literal grain fillers count as "placeholder", not asset-reuse). Fixed at the root: Vimeo now gets the same soft-probe circuit-breaker TikTok already has (`isVimeoMotionCandidate`, wired into both the `injectClip` top-up path and the `tryKeepVideoAsset` keep path) — once the first Vimeo proxy clip fails a cheap `yt-dlp --skip-download` probe, the run stops trusting further Vimeo clips and falls back to Archive/direct/Dailymotion motion instead of a doomed download that becomes a reused still. Also hard-rejected AI-generated-illustration/digital-art "family evicted" pads by content tell (`HOUSING_AI_ILLUSTRATION_RE`: "AI-generated", "midjourney", "digital painting", "artist's impression", etc.) rather than only by the shortform.com domain (already blocked in `affee65`) — the next AI-art blog under a different domain needed its own hard-reject too. **≥7 floors are unchanged** — this closes the harvest/soft-pass loophole that let an unverified-motion pool masquerade as upload-ready-adjacent; it does not relax any gate. No new score claimed — needs a fresh `generate:video` + `watch:video` run to confirm impact. **Housing remains open — do not mark it closed from this code fix alone.**
