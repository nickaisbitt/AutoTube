 ...
 * Uses evidence only (title/alt/url) — harvest `query` is aspirational and must
 * not mint a face from "shocked face eviction notice" stamped onto luxury tours
 * or packaging B-roll (housing-web78: mansions / panels / Sudan pads).
 */
...
export function passesHousingTimelineIntroEvidence(asset) {
  ...
  // housing-web82: promo flyer stills cleared person+eviction stamps — require motion.
...
export function buildEditTimeline(project, options = {}) {
  ...
  if (uniqueVideos.length > 0 && totalDur > 0 && cut > 0) {
    ...
    // housing-web153: rich DM/Archive pools (≥12 URLs) were excluded from the
    // snappy hold cap and felt like a TV news package. Cap rich housing like
    // other topics; medium housing (4–11 URLs) still lengthens holds so the
    // first-15s ≤1-reuse gate can cover without 2× loops.
  ...
  for (const seg of project.script || []) {
    ...
    const scoreAsset = (a, activeBeat = null, { ignoreReuse = false } = {}) => {
      ...
      // housing-web61: FKA Twigs / unrelated music pads harvested via face queries;
      // hard-reject at body-score level so they cannot pad any segment.
      if (topicIsHousing && /\bfka\s*twigs?\b/i.test(blob)) return -10;
      ...
      if (topicIsHealthcare && !coldEval) {
        const evBlob = assetEvidenceBlob(a);
        const isConferenceInterview =
          /\b(video\s+interview|in\s+this\s+video\s+interview|speaks?\s+with|(?:rsna|himss|ces)\s+\d{4}|imaging\s+wire|ramsoft|workflow\s+automation\s+at\s+rsna|mri\s+wide\s+bore\s+video|southeastern\s+spine|diversified\s+radiology\s+breast|mri\s+facility|open\s+bore\s+mri\s+(?:open|machine|video)|closed\s+mri|mri\s+machines\s+these|patient\s+friendly\s+video\s+was\s+created|aortic\s+aneurysm\s+and\s+imaging\s+patient)\b/i.test(evBlob);
        const hasStrongClinical =
          /\b(operating\s+room|or\s+lights?|surgical\s+robot|da\s*vinci\s+surg|radiologist\s+(?:screen|workstation|reads?)|mri\s+scan\s+performed|intraoperative|laparoscop)\b/i.test(evBlob);
        if (isConferenceInterview && !hasStrongClinical) reusePenalty -= 8;
      }
    ...
    while (t < duration - 0.05) {
      ...
      const canUseCandidate = (candidate, { allowOverReuse = false, relaxed = false } = {}) => {
        ...
        // Healthcare intro 0–3s: personal-injury clinic ads / STEM promos /
        // fundraising appeals / patient-portal UI stay banned even on the relaxed
        // pass so a real surgical-robot/OR clip always leads, never junk.
        if (
          introLeadWindow
          && topicIsHealthcare
          && isRejectedIntroLeadVisual(candidate, { healthcare: true })
        ) {
          return false;
        }
        ...
        if (!relaxed) {
          ...
          // housing-web82: do NOT exempt promo/LinkedIn stills — they cleared
          // person+eviction stamps and dominated thumbnail-still fallbacks.
          if (
            globalStartSec < FIRST_WINDOW_SEC
            && preferVideo
