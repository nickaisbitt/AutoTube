...
export function introFaceTier(asset, { airline = false, housing = false, healthcare = false } = {}) {
  ...
  if (healthcare) {
    ...
    const scienceNationClinical =
      /\bscience\s+nation\b/i.test(blob)
      && /\b(operating\s+room|surgical\s*robot|robot(?:ic)?\s*surger|robots?\s+changing\s+surgery|or\s+lights?|patient|surgeon|da\s*vinci|intraoperative|or\s+demonstration)\b/i.test(blob)
      && !(
        /\b(national\s+science\s+foundation|\bnsf\b)\b/i.test(blob)
        && !/\b(operating\s+room|or\s+lights?|patient|surgeon|da\s*vinci|or\s+demonstration)\b/i.test(blob)
      );
    ...
    const scoreAsset = (a, activeBeat = null, { ignoreReuse = false } = {}) => {
      ...
      if (isIntro || isOutro) {
        ...
        if (isIntro && topicIsHealthcare) {
          ...
          if (/\b(surgical\s*robot(?:ics?)?|operating\s+room|or\s+lights?|science\s+nation|onyx\s*rad|mri\s+(?:scan|performed|machine|room)|radiolog\w*\s+(?:ai|demo|demonstrat)|ai\s+radiolog|laparoscop\w*|intraoperative|da\s*vinci\s+surger)\b/i.test(evBlob)) {
            score += 4;
          }
        }
