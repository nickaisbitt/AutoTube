...
function isRejectedIntroLeadVisual(asset, { airline = false, housing = false, healthcare = false } = {}) {
  ...
  // housing-web18/19/20: Rolfe/Periscope/kids/sitcom, Progress Center podium,
...
export function introFaceTier(asset, { airline = false, housing = false, healthcare = false } = {}) {
  ...
  if (healthcare) {
    ...
    const clinicianScreenOrOr =
      /\b(ai\s+radiolog|radiolog\w*\s+ai|surgical\s*robot(?:ics?)?|robot(?:ic)?\s*surger|da\s*vinci\s*(?:surg|robot|OR)|cnbc\s+(?:surgical|robot|da\s*vinci|diagnos)|science\s+nation\s+(?:surgical|robot|radiol|hospital|medical)|onyx\s*rad(?:\s+ai|\s+radiol)?|ultrasound\s+(?:demo|demonstration)|mri\s+(?:monitor|screen)|pointing\s+at\s+(?:the\s+)?(?:monitor|screen)|operating\s+room|or\s+(?:suite|table|lights?)|radiologist\s+(?:workstation|screen|monitor|reads?|reviewing)|tiny\s+incision|hsc.{0,20}surgical\s+robot)\b/i.test(blob)
      || (
