...
export function checkEditTimelineIntroFace(project) {
  ...
  if (housing) {
    const hasFaceOrLivedIn = introAssets.some((asset) => {
      ...
      // Require lived-in / emotional face signal — bare "face" matches dartboard
      // "target face" and music-video metadata (housing-web81).
      return /\b(worried|shocked|crying|stressed|tenant|evict(?:ion|ed)?|apartment\s+interior|living\s+room|family\s+(?:crying|distressed|evict)|distressed\s+(?:family|tenant|person)|close[\s-]?up\s+(?:face|tenant|person)|tenant\s+face|person\s+face|people\s+(?:crying|evict|distressed))\b/i.test(evidence);
    });
    if (!hasFaceOrLivedIn) {
      return {
        pass: false,
