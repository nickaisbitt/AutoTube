...
export const HOUSING_OFF_TOPIC_BROLL_RE =
  /\b(?:car\s+crash|traffic\s+accident|auto(?:mobile)?\s+accident|dashcam(?:\s+(?:crash|footage|video))?|vehicle\s+(?:collision|wreck|crash)|wrecked\s+car|highway\s+accident|road\s+accident|pile[\s-]?up|car\s+wreck|crash\s+footage|accident\s+(?:footage|scene)|wildfire|forest\s+fires?|house\s+fire|apartment\s+fire|structure\s+fire|building\s+(?:on\s+)?fire|burning\s+(?:building|house|home|apartment)|fire\s+footage|airstrike|air\s*strike|war\s+zone|vietnam\s+war|war\s+home\s+movie|gaza(?:\s+strip)?|deir\s+al\s+balah|city\s+council|council\s+meeting|city\s+hall|public\s+(?:hearing|meeting)|town\s+hall(?:\s+meeting)?|board\s+meeting|building\s+commission|zoning\s+(?:board|hearing|commission)|commission\s+hearing|ribbon\s+cutting|apartments?\s+approved|earthquake|quake|tsunami|pie\s+chart|poll\s+graphic|opinion\s+poll|infographic|lending\s*tree|bar\s+chart|can\s*tv|station\s+id|satellite\s+map|digital\s+globe|house\s+on\s+(?:a\s+)?rock|floating\s+(?:rock|island)|3d\s+house|neohomeloans|housing\s+market\s+crash\.jpg|will-the-housing-market-crash|queen\s+elizabeth|royal\s+memorial)\b/i;
