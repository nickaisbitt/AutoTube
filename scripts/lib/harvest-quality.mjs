...
export function checkIntroFacePool(project) {
  ...
  if (healthcare) {
    const hasIntroFace = videos.some((asset) => {
      ...
      if (
        /\b(expo\s+(?:floor|booth|hall|suit)|exhibition\s+hall|trade\s*show\b|\bhimss\b|conference\s+(?:floor|booth|expo)|suit\s+(?:walk|drop|enter|stroll)|business\s+suit\s+(?:walk|stroll|enter)|blurry\s+test\s*tube|test\s*tube\s+(?:close\s*up|b-?roll|stock|only)|petri\s+dish|corporate\s+presentation)\b/i.test(blob)
