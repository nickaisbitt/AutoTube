...
export function checkIntroFacePool(project) {
  ...
  if (healthcare) {
    const hasIntroFace = videos.some((asset) => {
      const blob = [asset?.query, asset?.alt, asset?.title, asset?.source, asset?.url]
        .filter(Boolean).join(' ').toLowerCase();
      // Expo/suit/conference floor shots must not qualify as intro tier even if they
      // cleared the main junk filter — prefer OR/Science Nation over expo-floor walk-ons
      // (web17 hook: chest closeup then expo suit drop).
      // Also exclude blurry test-tube / petri-dish (healthcare-web18) and standalone
      // trade-show / HIMSS (healthcare-web20) from counting as intro evidence.
      // healthcare-web43: GeekBeat/Jackthreads/Bayer-logo/MLK/innovate talking-heads
      // must not count as clinical intro even when the harvest *query* said surgical robot.
      if (
        /\b(expo\s+(?:floor|booth|hall|suit)|exhibition\s+hall|trade\s*show\b|\bhimss\b|conference\s+(?:floor|booth|expo)|suit\s+(?:walk|drop|enter|stroll)|business\s+suit\s+(?:walk|stroll|enter)|blurry\s+test\s*tube|test\s*tube\s+(?:close\s*up|b-?roll|stock|only)|petri\s+dish|corporate\s+presentation|jackthreads|geekbeat(?:\.tv)?|unlock(?:ing)?\s+(?:your\s+)?(?:old\s+)?iphone|\bmlk\b|martin\s+luther\s+king|why\s+america\s+may\s+go\s+to\s+hell|this\s+or\s+that|metro\s+edition|che\s+guevara|imperialism|palestine\s+deepdive|bald\s+truth|why\s+do\s+i\s+innovate|hair\s+transplant|artas\s+hair|sri\s+ponni|bayer\s+(?:logo|booth|keynote|presentation)|corporate\s+logo\s+interview)\b/i.test(blob)
        && !/\b(surgical\s*robot|operating\s+room|or\s+lights?|clinical\s+use|patient|surgeon)\b/i.test(blob)
      ) return false;
      // Science Nation documentary / OR / surgical-robot motion qualifies as tier-0 intro.
      const clinicianScreenOrOr =
        /\b(science\s+nation|ai\s+radiolog|radiolog\w*\s+ai|surgical\s*robot|robot(?:ic)?\s*surger|da\s*vinci\s*(?:surg|robot|or)|ultrasound\s+(?:demo|demonstration)|mri\s+(?:monitor|screen)|operating\s+room|radiologist\s+(?:workstation|screen|monitor|reads?|reviewing))\b/i.test(blob)
        || (
