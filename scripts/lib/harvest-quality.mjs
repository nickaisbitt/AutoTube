...
export function checkEditTimelineIntroFace(project) {
  ...
  if (housing) {
    const hasFaceOrLivedIn = introAssets.some((asset) => {
      ...
      return /\b(face|worried|shocked|crying|stressed|person|people|tenant|evict|apartment\s+interior|living\s+room|family)\b/i.test(evidence);
    });
    if (!hasFaceOrLivedIn) {
      return {
        pass: false,
