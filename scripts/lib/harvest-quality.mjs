...
export function checkIntroFacePool(project) {
  ...
  if (healthcare) {
    const hasIntroFace = videos.some((asset) => {
      ...
      if (
        /\b(expo\s+(?:floor|booth|hall|suit)|exhibition\s+hall|trade\s*show|conference\s+(?:floor|booth|expo)|suit\s+(?:walk|drop|enter|stroll)|business\s+suit\s+(?:walk|stroll|enter))\b/i.test(blob)
